...
class AdaptiveIntervalInterpolation(PolynomialInterpolation):
    ...
    def forward_values(self, input):
        if self.has_exported:
            output = super(AdaptiveIntervalInterpolation, self).forward_values(input)
            return output
    ...
    def to(self, *args, **kwargs):
        super(AdaptiveIntervalInterpolation, self).to(*args, **kwargs)
...
class AdaptiveTunedValuesIntervalInterpolation(PolynomialInterpolation):
    # Support:
    def __init__(self, N=10, degree=1, bl=-10., br=10., device="cpu", dtype=torch.float32, *args, **kwargs):
        super(AdaptiveTunedValuesIntervalInterpolation, self).__init__( N=N, degree=degree, bl=bl, br=br, device=device, dtype=dtype, *args, **kwargs)
    ...
    def forward_values(self, input):
        if self.has_exported:
            output = super(AdaptiveTunedValuesIntervalInterpolation, self).forward_values(input)
            return output
    ...
    def to(self, *args, **kwargs):
        super(AdaptiveTunedValuesIntervalInterpolation, self).to(*args, **kwargs)
        self.sample_points_buffer = self.sample_points_buffer.to(*args, **kwargs)
        self.bl = self.bl.to(*args, **kwargs)
        self.br = self.br.to(*args, **kwargs)
